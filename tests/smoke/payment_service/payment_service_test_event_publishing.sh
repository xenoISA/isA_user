...
EVENT_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} --tail=50 | grep "Published payment.intent.created" || echo "")
...
if [ -n "$PAYMENT_INTENT_ID" ] && [ "$PAYMENT_INTENT_ID" != "null" ]; then
    ...
    EVENT_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} --tail=50 | grep "Published payment.completed" || echo "")
...
if [ -n "$SUBSCRIPTION_ID" ] && [ "$SUBSCRIPTION_ID" != "null" ]; then
    ...
    EVENT_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} --tail=50 | grep "Published subscription.canceled" || echo "")
...
HANDLER_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} | grep "Subscribed to event\|Registered handler" || echo "")
