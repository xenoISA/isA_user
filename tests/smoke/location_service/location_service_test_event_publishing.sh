...
if echo "$RESPONSE" | grep -q '"success":true'; then
    ...
    echo -e "${YELLOW}⚠ Geofence creation skipped (table not migrated)${NC}"
    echo -e "${YELLOW}Note: This is optional - geofence feature not deployed${NC}"
    PASSED_3=1  # Mark as passed since it's optional
fi
echo ""
