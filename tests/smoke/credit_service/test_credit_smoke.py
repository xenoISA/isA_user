...
class TestHealthSmoke:
    ...
    async def test_health_endpoint_responds(self, http_client, credit_health_url):
        """SMOKE: GET /health returns 200"""
        response = await http_client.get(credit_health_url)
        assert (
    ...
    async def test_api_health_alias_responds(self, http_client, credit_api_v1):
        """SMOKE: GET /api/v1/credits/health returns 200"""
        response = await http_client.get(f"{credit_api_v1}/health")
        assert (
            response.status_code == 200
        ), f"API health check failed: {response.status_code}"

        data = response.json()
        assert "status" in data, "API health response missing status field"
...
class TestAccountOperationsSmoke:
    ...
    async def test_create_account_works(
        self, http_client, credit_api_v1, test_data_registry
    ):
        ...
        request_data = CreditTestDataFactory.make_create_account_request(
            user_id=user_id, credit_type=CreditTypeEnum.BONUS.value
        )

        response = await http_client.post(
            f"{credit_api_v1}/accounts", json=request_data.model_dump(mode="json")
        )
    ...
    async def test_get_accounts_works(self, http_client, credit_api_v1):
        ...
        response = await http_client.get(
            f"{credit_api_v1}/accounts", params={"user_id": user_id}
        )
        ...
        if response.status_code == 200:
            data = response.json()
            assert "accounts" in data or isinstance(
                data, list
...
class TestAllocationSmoke:
    ...
    async def test_allocate_credits_works(
        self, http_client, credit_api_v1, test_data_registry
    ):
        ...
        request_data = CreditTestDataFactory.make_allocate_credits_request(
            user_id=user_id, credit_type=CreditTypeEnum.BONUS.value, amount=1000
        )

        response = await http_client.post(
            f"{credit_api_v1}/allocate", json=request_data.model_dump(mode="json")
        )
    ...
    async def test_allocate_credits_rejects_invalid_amount(
        self, http_client, credit_api_v1
    ):
...
class TestConsumptionSmoke:
    ...
    async def test_consume_credits_works(
        self, http_client, credit_api_v1, test_data_registry
    ):
        ...
        response = await http_client.post(
            f"{credit_api_v1}/consume", json=request_data.model_dump(mode="json")
        )
    ...
    async def test_consume_credits_rejects_invalid_user(
        self, http_client, credit_api_v1
    ):
        """SMOKE: POST /consume rejects empty user_id"""
        response = await http_client.post(
            f"{credit_api_v1}/consume",
            json={"user_id": "", "amount": 100},  # Invalid: empty user_id
        )
...
class TestBalanceSmoke:
    ...
    async def test_get_balance_works(self, http_client, credit_api_v1):
        ...
        response = await http_client.get(
            f"{credit_api_v1}/balance", params={"user_id": user_id}
        )
        ...
        if response.status_code == 200:
            data = response.json()
            assert (
                "total_balance" in data or "user_id" in data
...
class TestTransferSmoke:
    ...
    async def test_transfer_credits_works(
        self, http_client, credit_api_v1, test_data_registry
    ):
        ...
        response = await http_client.post(
            f"{credit_api_v1}/transfer", json=request_data.model_dump(mode="json")
        )
...
class TestCampaignSmoke:
    ...
    async def test_create_campaign_works(
        self, http_client, credit_api_v1, test_data_registry
    ):
        ...
        response = await http_client.post(
            f"{credit_api_v1}/campaigns", json=request_data.model_dump(mode="json")
        )
...
class TestTransactionHistorySmoke:
    ...
    async def test_get_transactions_works(self, http_client, credit_api_v1):
        ...
        if response.status_code == 200:
            data = response.json()
            assert "transactions" in data or isinstance(
                data, list
            ), "Transactions response missing expected structure"

    async def test_get_transactions_validates_pagination(
        self, http_client, credit_api_v1
    ):
...
class TestStatisticsSmoke:
    ...
    async def test_get_statistics_works(self, http_client, credit_api_v1):
        ...
        if response.status_code == 200:
            ...
            assert has_stats or isinstance(
                data, dict
