#!/bin/bash
# Test Event Subscriptions - Verify invitation_service can handle incoming events
...
echo -e "${CYAN}          Invitation Service${NC}"
echo -e "${CYAN}======================================================================${NC}"
echo ""

# Check if we're running in K8s
NAMESPACE="isa-cloud-staging"
if ! kubectl get pods -n ${NAMESPACE} -l app=invitation &> /dev/null; then
    echo -e "${RED}✗ Cannot find invitation pods in Kubernetes${NC}"
    echo "Please ensure the service is deployed to K8s"
    exit 1
fi

echo -e "${BLUE}✓ Found invitation pods in Kubernetes${NC}"
echo ""

# Get the invitation pod name
POD_NAME=$(kubectl get pods -n ${NAMESPACE} -l app=invitation -o jsonpath='{.items[0].metadata.name}')
...
HANDLER_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} --tail=500 2>/dev/null | grep -iE "(Subscribed to event|event.bus|handler|Event bus)" || echo "")

if [ -n "$HANDLER_LOGS" ]; then
    echo -e "${GREEN}✓ SUCCESS: Event-related logs found!${NC}"
    echo "$HANDLER_LOGS" | head -10
    PASSED_1=1
else
    # Even if no specific logs found, check if service is running with handlers module
...
# =============================================================================
# Test 2: Verify organization.deleted Event Handling
# =============================================================================
echo -e "${YELLOW}=====================================================================${NC}"
echo -e "${YELLOW}Test 2: organization.deleted Event Handling${NC}"
echo -e "${YELLOW}=====================================================================${NC}"
echo ""

echo -e "${BLUE}This test would verify organization.deleted event handling by:${NC}"
echo -e "  1. Creating invitations for a test organization"
echo -e "  2. Triggering organization.deleted event from organization_service"
echo -e "  3. Monitoring invitation-service logs for event processing"
echo -e "  4. Verifying all pending invitations were cancelled"
echo ""
...
echo -e "  1. Creating invitations sent by a test user"
echo -e "  2. Triggering user.deleted event from account_service"
echo -e "  3. Monitoring invitation-service logs for event processing"
echo -e "  4. Verifying invitations sent by the user were cancelled"
echo ""
...
echo "Test 2: organization.deleted handling - $([ $PASSED_2 -eq 1 ] && echo -e "${GREEN}✓ PASSED${NC}" || echo -e "${RED}✗ FAILED${NC}")"
...
if [ $TOTAL_PASSED -eq $TOTAL_TESTS ]; then
    ...
    echo -e "  ${GREEN}✓${NC} organization.deleted - Handler registered"
    ...
    echo "2. Check invitation logs: kubectl logs -n ${NAMESPACE} ${POD_NAME}"
