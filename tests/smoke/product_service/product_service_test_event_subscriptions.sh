...
echo -e "${YELLOW}======================================================================"
echo -e "Test 2: Event Bus Connection"
echo -e "======================================================================${NC}"
echo ""

echo -e "${BLUE}Checking if product service connected to NATS event bus${NC}"
BUS_LOGS=$(kubectl logs -n ${NAMESPACE} ${POD_NAME} | grep -i "event bus\|nats\|connected" | head -5 || echo "")
...
if [ -n "$BUS_LOGS" ]; then
    ...
    echo -e "${YELLOW}⚠ WARNING: No event bus logs found${NC}"
    echo -e "${YELLOW}Note: Event bus may be connected but logs rotated${NC}"
