...
def mock_service():
    service = MagicMock()
    service.list_project_files = AsyncMock(return_value=[
        {
            "id": "file_1",
            "project_id": "proj1",
            "filename": "guide.md",
    ...
    service.upload_project_file = AsyncMock(return_value={
        "id": "file_1",
        "project_id": "proj1",
        "filename": "guide.md",
        "file_type": "text/markdown",
...
class TestProjectFileEndpoints:
    async def test_list_project_files_returns_files_and_total(self, client, mock_service):
        response = await client.get("/api/v1/projects/proj1/files")

        assert response.status_code == 200
        assert response.json()["total"] == 1
        assert response.json()["files"][0]["filename"] == "guide.md"
        mock_service.list_project_files.assert_awaited_once_with("proj1", "user1", 100, 0)

    async def test_upload_project_file_accepts_multipart_file(self, client, mock_service):
