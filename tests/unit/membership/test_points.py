...
class TestEarnPoints:
    ...
    async def test_earn_points_success(self, membership_service, sample_membership):
        ...
        result = await membership_service.earn_points(
            user_id="user_123", points_amount=500, source="order_completed"
        )
    ...
    async def test_earn_points_gold_tier_multiplier(
        self, membership_service, gold_membership
    ):
        """Test gold tier gets 1.5x multiplier"""
        result = await membership_service.earn_points(
            user_id="user_gold", points_amount=1000, source="order_completed"
        )
    ...
    async def test_earn_points_no_membership(self, membership_service):
        ...
        result = await membership_service.earn_points(
            user_id="nonexistent_user", points_amount=500, source="order"
        )

        assert result.success is False
        assert "no membership found" in result.message.lower()

    @pytest.mark.asyncio
    async def test_earn_points_suspended_membership(
        self, membership_service, mock_repository, sample_membership
    ):
        """Test earning points with suspended membership fails"""
        await mock_repository.update_status(
            sample_membership.membership_id, MembershipStatus.SUSPENDED
        )

        result = await membership_service.earn_points(
            user_id="user_123", points_amount=500, source="order"
        )
    ...
    @pytest.mark.asyncio
    async def test_earn_points_expired_membership(
        self, membership_service, mock_repository, sample_membership
    ):
        """Test earning points with expired membership fails"""
        await mock_repository.update_status(
            sample_membership.membership_id, MembershipStatus.EXPIRED
        )
    ...
    async def test_earn_points_with_reference_id(
        self, membership_service, sample_membership, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_earn_points_with_description(
        self, membership_service, sample_membership, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_earn_points_updates_tier_points(
        self, membership_service, sample_membership, mock_repository
    ):
    ...
    async def test_earn_points_publishes_event(
        self, membership_service, sample_membership, mock_event_bus
    ):
    ...
    async def test_earn_small_points(self, membership_service, sample_membership):
        ...
        result = await membership_service.earn_points(
            user_id="user_123", points_amount=1, source="daily_login"
        )
...
class TestRedeemPoints:
    ...
    async def test_redeem_points_success(self, membership_service, sample_membership):
        ...
        result = await membership_service.redeem_points(
            user_id="user_123", points_amount=500, reward_code="DISCOUNT_10"
        )
    ...
    async def test_redeem_all_points(self, membership_service, sample_membership):
        ...
        result = await membership_service.redeem_points(
            user_id="user_123", points_amount=1000, reward_code="FULL_REDEEM"
        )
    ...
    async def test_redeem_insufficient_points(
        self, membership_service, sample_membership
    ):
    ...
    async def test_redeem_no_membership(self, membership_service):
        ...
        result = await membership_service.redeem_points(
            user_id="no_member", points_amount=100, reward_code="DISCOUNT"
        )
    ...
    @pytest.mark.asyncio
    async def test_redeem_suspended_membership(
        self, membership_service, mock_repository, sample_membership
    ):
        ...
        result = await membership_service.redeem_points(
            user_id="user_123", points_amount=100, reward_code="DISCOUNT"
        )
    ...
    @pytest.mark.asyncio
    async def test_redeem_creates_history(
        self, membership_service, sample_membership, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_redeem_publishes_event(
        self, membership_service, sample_membership, mock_event_bus
    ):
...
class TestGetPointsBalance:
    ...
    async def test_get_balance_success(self, membership_service, sample_membership):
        ...
        result = await membership_service.get_points_balance(user_id="user_123")
    ...
    async def test_get_balance_no_membership(self, membership_service):
        ...
        result = await membership_service.get_points_balance(user_id="no_member")

        assert result.success is False
        assert result.balance is None

    @pytest.mark.asyncio
    async def test_get_balance_includes_tier_info(
        self, membership_service, sample_membership
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_balance_with_organization(
        self, membership_service, mock_repository
    ):
        ...
        result = await membership_service.get_points_balance(
            user_id="org_user", organization_id="org_123"
        )
...
class TestTierMultipliers:
    ...
    async def test_bronze_multiplier_is_1x(self, membership_service, mock_repository):
        ...
        await mock_repository.create_membership(
            user_id="bronze_user", tier_code="bronze", points_balance=0
        )

        result = await membership_service.earn_points(
            user_id="bronze_user", points_amount=100, source="test"
        )
    ...
    @pytest.mark.asyncio
    async def test_silver_multiplier_is_1_25x(
        self, membership_service, mock_repository
    ):
        """Test silver tier has 1.25x multiplier"""
        await mock_repository.create_membership(
            user_id="silver_user", tier_code="silver", points_balance=0
        )

        result = await membership_service.earn_points(
            user_id="silver_user", points_amount=100, source="test"
        )
    ...
    async def test_gold_multiplier_is_1_5x(self, membership_service, mock_repository):
        ...
        await mock_repository.create_membership(
            user_id="gold_user", tier_code="gold", points_balance=0
        )

        result = await membership_service.earn_points(
            user_id="gold_user", points_amount=100, source="test"
        )
    ...
    async def test_platinum_multiplier_is_2x(self, membership_service, mock_repository):
        ...
        await mock_repository.create_membership(
            user_id="plat_user", tier_code="platinum", points_balance=0
        )

        result = await membership_service.earn_points(
            user_id="plat_user", points_amount=100, source="test"
        )
    ...
    async def test_diamond_multiplier_is_3x(self, membership_service, mock_repository):
        ...
        await mock_repository.create_membership(
            user_id="diamond_user", tier_code="diamond", points_balance=0
        )

        result = await membership_service.earn_points(
            user_id="diamond_user", points_amount=100, source="test"
        )
