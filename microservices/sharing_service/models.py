...
class Share(BaseModel):
    ...
    permissions: str = Field(
        default=SharePermission.VIEW_ONLY, description="Permission level"
    )
    session_snapshot: Optional[Dict[str, Any]] = Field(
        None,
        description="Immutable session metadata captured when the share is created",
    )
    messages_snapshot: Optional[List[Dict[str, Any]]] = Field(
        None,
        description="Immutable message transcript captured when the share is created",
    )
    expires_at: Optional[datetime] = Field(
        None, description="Expiry timestamp (null = never)"
    )
