...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"⚠️  Failed to initialize centralized event bus: {e}. Continuing without event publishing."
            )
            event_bus = None

        # 初始化事件服务（EventService 会自己初始化 repository）
        print("[event-service] Initializing event service...")
        ...
        print("[event-service] Using centralized event bus (NATS via gRPC)")
        nats_client = None
        js = None
        ...
        print(
            f"[event-service] Service started successfully on port {config.service_port}"
        )
        ...
        print("[event-service] Shutting down...")
...
app = FastAPI(
    title="Event Service", description="统一事件管理服务", version="1.0.0", lifespan=lifespan
)
app.add_middleware(shutdown_middleware, shutdown_manager=shutdown_manager)
setup_metrics(app, "event_service")
...
health = HealthCheck(
    "event_service", version="1.0.0", shutdown_manager=shutdown_manager
)
...
async def create_event(
    ...
    try:
        ...
        if nats_client and js:
            background_tasks.add_task(publish_event_to_nats, event)
...
async def list_subscriptions(service: EventService = Depends(get_event_service)):
    """列出所有订阅"""
    try:
...
async def delete_subscription(
    subscription_id: str, service: EventService = Depends(get_event_service)
):
    """删除订阅"""
    try:
...
class FrontendEvent(FrontendBaseModel):
    ...
    category: str = (
        "user_interaction"  # 'user_interaction', 'business_action', 'system_event'
    )
...
async def collect_frontend_event(event: FrontendEvent, request: Request):
    """采集单个前端事件"""
    try:
        # 添加客户端信息
        client_info = {
        ...
        if nats_client and js:
            ...
            return {"status": "error", "message": "Event stream not available"}
...
async def collect_frontend_events_batch(batch: FrontendEventBatch, request: Request):
    """批量采集前端事件 - 高性能处理"""
    try:
...
async def replay_events(
    ...
    try:
        # 在后台执行重放
        background_tasks.add_task(service.replay_events, request)
...
async def get_projection(
    entity_type: str, entity_id: str, service: EventService = Depends(get_event_service)
):
    """获取实体投影"""
    try:
        ...
        if isinstance(body, list):
            for event_data in body:
                ...
                background_tasks.add_task(
                    service.create_event_from_rudderstack, rudderstack_event
                )
        else:
...
async def list_processors(service: EventService = Depends(get_event_service)):
    """列出所有处理器"""
    try:
...
async def toggle_processor(
    ...
    try:
        ...
        return {"status": "updated", "processor_id": processor_id, "enabled": enabled}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
