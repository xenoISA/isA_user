...
class LocationService:
    ...
    async def report_location(
        self, request: LocationReportRequest, user_id: str
    ) -> LocationOperationResult:
        """
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Database error creating location: {db_error}", exc_info=True
                )
            ...
            if result:
                ...
                logger.warning(
                    f"create_location returned None for location_id: {location_id}"
                )
                return LocationOperationResult(
                    success=False,
                    operation="report_location",
            ...
            return LocationOperationResult(
                success=False, operation="report_location", message=f"Error: {str(e)}"
            )

    async def batch_report_locations(
        self, request: LocationBatchRequest, user_id: str
    ) -> LocationOperationResult:
        """
    ...
    async def get_user_devices_locations(self, user_id: str) -> List[Dict[str, Any]]:
    ...
    async def create_geofence(
        ...
        try:
            ...
            if (
                request.shape_type == GeofenceShapeType.POLYGON
                and not request.polygon_coordinates
            ):
                return LocationOperationResult(
                    success=False,
                    operation="create_geofence",
            ...
            return LocationOperationResult(
                success=False, operation="create_geofence", message=f"Error: {str(e)}"
            )

    async def get_geofence(
        self, geofence_id: str, user_id: str
    ) -> Optional[Dict[str, Any]]:
        """Get geofence by ID"""
        try:
            ...
            if geofence and geofence["user_id"] != user_id:
                logger.warning(
                    f"User {user_id} attempted to access geofence {geofence_id}"
                )
                return None
    ...
    async def update_geofence(
        self, geofence_id: str, request: GeofenceUpdateRequest, user_id: str
    ) -> LocationOperationResult:
        """Update geofence"""
        try:
            # Verify ownership
            ...
            return LocationOperationResult(
                success=False, operation="update_geofence", message="No updates applied"
            )

        except Exception as e:
            logger.error(f"Error updating geofence: {e}")
            return LocationOperationResult(
                success=False, operation="update_geofence", message=f"Error: {str(e)}"
            )
    ...
    async def delete_geofence(
        ...
        try:
            ...
            return LocationOperationResult(
                success=False, operation="delete_geofence", message=f"Error: {str(e)}"
            )
    ...
    async def _toggle_geofence(
        self, geofence_id: str, user_id: str, active: bool
    ) -> LocationOperationResult:
        ...
        try:
            ...
            return LocationOperationResult(
                success=False, operation="toggle_geofence", message=f"Error: {str(e)}"
            )
    ...
    async def find_nearby_devices(
        self, request: NearbySearchRequest, user_id: str
    ) -> List[Dict[str, Any]]:
    ...
    async def search_radius(
        self, request: RadiusSearchRequest, user_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def calculate_distance(
        ...
        distance_m = LocationRepository.calculate_distance(
            from_lat, from_lon, to_lat, to_lon
        )
        return {"distance_meters": distance_m, "distance_km": distance_m / 1000}

    # ==================== Place Operations ====================

    async def create_place(
        self, request: PlaceCreateRequest, user_id: str
    ) -> LocationOperationResult:
        """Create a new place"""
        try:
            ...
            return LocationOperationResult(
                success=False, operation="create_place", message=f"Error: {str(e)}"
            )

    async def get_place(self, place_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Get place by ID"""
        try:
    ...
    async def update_place(
        self, place_id: str, request: PlaceUpdateRequest, user_id: str
    ) -> LocationOperationResult:
        """Update place"""
        try:
            # Verify ownership
            ...
            return LocationOperationResult(
                success=False, operation="update_place", message="No updates applied"
            )

        except Exception as e:
            logger.error(f"Error updating place: {e}")
            return LocationOperationResult(
                success=False, operation="update_place", message=f"Error: {str(e)}"
            )

    async def delete_place(
        self, place_id: str, user_id: str
    ) -> LocationOperationResult:
        """Delete place"""
        try:
            # Verify ownership
            ...
            return LocationOperationResult(
                success=False, operation="delete_place", message=f"Error: {str(e)}"
            )
