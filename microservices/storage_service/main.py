    ...
    if event_bus:
        try:
            ...
            intelligence_service = getattr(
                storage_service, "intelligence_service", None
            )

            handler_map = get_event_handlers(
                storage_service, intelligence_service, event_bus
            )
            ...
            logger.info(
                f"✅ Storage event subscriber started ({len(handler_map)} event patterns)"
            )
...
health = HealthCheck(
    "storage_service", version="1.0.0", shutdown_manager=shutdown_manager
)
health.add_nats(lambda: event_bus)
health.add_minio(lambda: storage_service.minio_client if storage_service else None)
