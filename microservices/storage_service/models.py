...
重构说明：
- 移除了照片版本管理模型（现由 Media Service 负责）
- 移除了相册管理模型（现由 Album Service 负责）
- 移除了播放列表和轮播计划模型（现由 Media Service 负责）
- 移除了智能照片选择和缓存模型（现由 Media Service 负责）
- 保留核心存储功能：文件、分享、配额
"""
...
class StoredFile(BaseModel):
    ...
    storage_provider: StorageProvider = Field(
        StorageProvider.MINIO, description="存储提供商"
    )
    ...
    access_level: FileAccessLevel = Field(
        FileAccessLevel.PRIVATE, description="访问级别"
    )
...
class FileUploadRequest(BaseModel):
    ...
    enable_indexing: bool = Field(
        True, description="Enable auto-indexing for RAG/vectorization"
    )
