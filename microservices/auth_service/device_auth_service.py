...
class DeviceAuthService:
    ...
    def __init__(self, repository: DeviceAuthRepository, event_bus=None):
        ...
        self.jwt_secret = os.getenv("JWT_SECRET") or os.getenv("LOCAL_JWT_SECRET")
        if not self.jwt_secret:
            # Keep local-dev startup working while still warning loudly.
            self.jwt_secret = secrets.token_urlsafe(64)
            logger.warning(
                "JWT_SECRET not set for device authentication; using ephemeral dev secret."
            )
            ...
            if result:
                ...
                if self.event_bus:
                    try:
                        ...
                        logger.info(
                            f"Published device.registered event for device {result['device_id']}"
                        )
    ...
    async def authenticate_device(
        ...
        try:
            ...
            if device:
                ...
                token = jwt.encode(
                    token_payload, self.jwt_secret, algorithm=self.jwt_algorithm
                )
                ...
                if self.event_bus:
                    try:
                        event = Event(
                            ...
                            metadata={"user_agent": user_agent or ""},
                        )
                        await self.event_bus.publish_event(event)
                        logger.info(
                            f"Published device.authenticated event for device {device['device_id']}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to publish device.authenticated event: {e}"
                        )
    ...
    async def refresh_device_secret(
        self, device_id: str, organization_id: str
    ) -> Dict[str, Any]:
        """刷新设备密钥"""
        try:
            # 验证设备归属
            ...
            result = await self.repository.update_device_credential(
                device_id, {"device_secret": new_secret_hash}
            )
            ...
            if result:
                ...
                return {"success": False, "error": "Failed to refresh device secret"}
    ...
    async def get_device_auth_logs(
        self, device_id: str, organization_id: str, limit: int = 100
    ) -> Dict[str, Any]:
        """获取设备认证日志"""
        try:
            # 验证设备归属
    ...
    async def verify_pairing_token(
        ...
        try:
            ...
            logger.info(
                f"Pairing token verified for device {device_id}, user {user_id}"
            )

            return {"valid": True, "device_id": device_id, "user_id": user_id}
