...
class TaskService:
    ...
    def __init__(self, event_bus=None, config_manager: Optional[ConfigManager] = None):
        ...
        self.config_manager = (
            config_manager if config_manager else ConfigManager("task_service")
        )
        self.consul = None
        self._init_consul()
    ...
    def _create_temp_orchestrator(self):
        ...
        class TempOrchestrator:
            async def get_user_context_with_permissions(self, user_id):
                return {
                    "success": True,
                    "user": {"user_id": user_id, "subscription_level": "basic"},
                }
    ...
    async def create_task(
        self, user_id: str, request: TaskCreateRequest
    ) -> TaskResponse:
        """创建用户任务"""
        try:
            logger.debug(
                f"Creating task for user {user_id}, type: {request.task_type.value}"
            )

            # 1. 获取用户信息和权限检查
            user_context = await self.orchestrator.get_user_context_with_permissions(
                user_id
            )
            if not user_context["success"]:
                raise TaskExecutionError(
                    "Failed to get user context", error_code="USER_CONTEXT_ERROR"
                )
            ...
            if not task:
                raise TaskExecutionError(
                    "Failed to create task", error_code="CREATION_FAILED"
                )
            ...
            logger.error(
                f"Failed to create task for user {user_id}: {e}", exc_info=True
            )
            raise TaskExecutionError(
                f"Task creation failed: {str(e)}", error_code="INTERNAL_ERROR"
            )
    ...
    async def update_task(
        self, task_id: str, user_id: str, request: TaskUpdateRequest
    ) -> TaskResponse:
        """更新任务"""
        try:
            # 权限检查
            ...
            if not existing_task:
                raise TaskExecutionError(
                    "Task not found", task_id=task_id, error_code="NOT_FOUND"
                )
            ...
            if request.schedule is not None:
                ...
                # 重新计算下次执行时间
                if request.schedule:
                    update_data["next_run_time"] = self._calculate_next_run_time(
                        request.schedule
                    )
            ...
            if not success:
                raise TaskExecutionError(
                    "Failed to update task", task_id=task_id, error_code="UPDATE_FAILED"
                )
            ...
            if success:
                ...
                if self.event_bus:
                    try:
                        ...
                        await self.event_bus.publish_event(event)
                        logger.info(
                            f"Published task.cancelled event for task {task_id}"
                        )
                    except Exception as e:
    ...
    async def execute_task(
        self, task_id: str, user_id: str, request: TaskExecutionRequest
    ) -> TaskExecutionResponse:
        """手动执行任务"""
        try:
            # 权限检查
            ...
            execution_record = await self.repository.create_execution_record(
                task_id=task_id, user_id=user_id, execution_data=request.dict()
            )

            if not execution_record:
                raise TaskExecutionError(
                    "Failed to create execution record", task_id=task_id
                )
            ...
            logger.info(
                f"Task execution started: {task_id} (execution: {execution_record.execution_id})"
            )
            ...
            raise TaskExecutionError(
                f"Task execution failed: {str(e)}", task_id=task_id
            )

    async def _execute_task_async(
        self, task: TaskResponse, execution_record: TaskExecutionResponse
    ):
        """异步执行任务"""
        try:
            # 更新任务状态为运行中
            await self.repository.update_task(
                task.task_id, task.user_id, {"status": TaskStatus.RUNNING.value}
            )
            ...
            await self.repository.update_execution_record(
                execution_record.execution_id, execution_result
            )
            ...
            await self.repository.update_task(
                task.task_id, task.user_id, {"status": new_status}
            )

            # 发送执行完成通知
            await self._send_execution_notification(
                task, execution_record, success=True, result=result
            )
            ...
            if self.event_bus:
                try:
                    ...
                    await self.event_bus.publish_event(event)
                    logger.info(
                        f"Published task.completed event for task {task.task_id}"
                    )
                except Exception as e:
            ...
            # 更新任务状态为失败
            await self.repository.update_task(
                task.task_id, task.user_id, {"status": TaskStatus.FAILED.value}
            )
            ...
            # 发送执行失败通知
            await self._send_execution_notification(
                task, execution_record, success=False, error=str(e)
            )
    ...
    async def get_task_templates(self, user_id: str) -> List[TaskTemplateResponse]:
        ...
        try:
            ...
            if user_context["success"] and user_context["user"]:
                subscription_level = user_context["user"].get(
                    "subscription_level", "free"
                )
    ...
    async def _check_task_access_permission(
        self, task_id: str, user_id: str, action: str
    ):
        """检查任务访问权限"""
        try:
            # 首先验证任务是否属于用户
    ...
    async def _check_user_access_permission(self, user_id: str, action: str):
        ...
        try:
            ...
            raise TaskExecutionError(
                "Permission check failed", error_code="PERMISSION_CHECK_ERROR"
            )
    ...
    async def _check_task_limits(self, user_id: str, subscription_level: str):
        ...
        try:
            ...
            if max_tasks > 0:  # -1 表示无限制
                user_tasks = await self.repository.get_user_tasks(
                    user_id, limit=max_tasks + 1
                )
            ...
            raise TaskExecutionError(
                "Task limit check failed", error_code="LIMIT_CHECK_ERROR"
            )
    ...
    async def _check_execution_limits(self, user_id: str, task: TaskResponse):
        ...
        try:
            ...
            raise TaskExecutionError(
                "Execution limit check failed", error_code="LIMIT_CHECK_ERROR"
            )
    ...
    async def _validate_task_config(self, request: TaskCreateRequest):
        ...
        try:
            ...
            if not request.name or len(request.name.strip()) == 0:
                raise TaskExecutionError(
                    "Task name cannot be empty", error_code="INVALID_CONFIG"
                )
            ...
            raise TaskExecutionError(
                f"Invalid task configuration: {str(e)}", error_code="INVALID_CONFIG"
            )
    ...
    def _validate_weather_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for weather task: {field}"
                )
    ...
    def _validate_news_monitor_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for news monitor task: {field}"
                )
    ...
    def _validate_weather_alert_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for weather alert task: {field}"
                )
    ...
    def _validate_price_tracker_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for price tracker task: {field}"
                )
    ...
    def _validate_reminder_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for reminder task: {field}"
                )
    ...
    def _validate_calendar_event_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for calendar event task: {field}"
                )
    ...
    def _validate_custom_config(self, config: Dict[str, Any]):
        ...
        for field in required_fields:
            if field not in config:
                raise TaskExecutionError(
                    f"Missing required field for custom task: {field}"
                )
            ...
            if schedule_type == "once":
                ...
                if now.month == 12:
                    next_month = now.replace(
                        year=now.year + 1, month=1, day=day_of_month
                    )
                else:
    ...
    async def _execute_weather_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Weather task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_news_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"News task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_news_monitor_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"News monitor task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_weather_alert_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            if "temperature" in alert_conditions:
                ...
                if temp_condition.get("operator") == ">" and current_weather[
                    "temperature"
                ] > temp_condition.get("value", 0):
                    alerts.append(
                        f"Temperature alert: {current_weather['temperature']}°C"
                    )
            ...
            raise TaskExecutionError(
                f"Weather alert task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_price_tracker_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Price tracker task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_todo_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Todo task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_reminder_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Reminder task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_calendar_event_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Calendar event task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _execute_custom_task(self, task: TaskResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TaskExecutionError(
                f"Custom task failed: {str(e)}", task_id=task.task_id
            )
    ...
    async def _schedule_reminder_notification(
        self, task: TaskResponse, reminder_time: datetime
    ):
        """安排提醒通知"""
        try:
            # 这里可以集成通知服务来安排定时通知
            logger.info(
                f"Reminder scheduled for task {task.task_id} at {reminder_time}"
            )
        except Exception as e:
    ...
    async def _send_execution_notification(
        ...
        try:
            notification_data = {
                ...
                "content": self._format_execution_notification(
            ...
            await self.communicator.call_notification_service(
                "/api/v1/notifications/send", notification_data, "POST"
            )
    ...
    def _format_execution_notification(
        self, task: TaskResponse, success: bool, result: Dict = None, error: str = None
    ) -> str:
        """格式化执行通知内容"""
        if success:
            content = f"""
    ...
    async def _send_weather_alert_notification(
        self, task: TaskResponse, alert_data: Dict
    ):
        """发送天气预警通知"""
        try:
    ...
    async def _send_reminder_notification(
        self, task: TaskResponse, reminder_data: Dict
    ):
        """发送提醒通知"""
        try:
            content = f"""
    ...
    async def _send_calendar_notification(
        self, task: TaskResponse, calendar_data: Dict
    ):
        """发送日历事件通知"""
        try:
            content = f"""
            ...
            await self._send_notification(
                task, "Calendar Reminder", content, calendar_data
            )
        except Exception as e:
            logger.error(f"Failed to send calendar notification: {e}")

    async def _send_notification(
        self, task: TaskResponse, subject: str, content: str, data: Dict
    ):
        """通用通知发送方法"""
        try:
            notification_data = {
    ...
    async def call_notification_service(
        ...
        """调用通知服务的便捷方法"""
        return await self.communicator._call_service(
            "notification", endpoint, data, method
        )
