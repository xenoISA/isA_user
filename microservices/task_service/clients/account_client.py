...
Client for task_service to interact with account_service.
Used for validating user existence before task operations.
"""
...
class AccountClient:
    """
    Wrapper client for Account Service calls from Task Service.
    ...
    async def validate_user_exists(self, user_id: str) -> bool:
        ...
        Used before creating tasks for users.
    ...
    async def get_user_info(self, user_id: str) -> Optional[dict]:
        """
        Get user information for task context.
    ...
    async def check_user_active(self, user_id: str) -> bool:
        ...
        Prevents task operations for suspended/deleted accounts.
    ...
    async def get_user_timezone(self, user_id: str) -> Optional[str]:
        """
        Get user timezone for task scheduling.
    ...
    async def get_user_notification_preferences(self, user_id: str) -> dict:
        """
        Get user notification preferences for task reminders.
        ...
        try:
            ...
            if profile:
                preferences = profile.get("preferences", {})
                return {
                    "email": preferences.get("notification_email", True),
                    "push": preferences.get("notification_push", True)
                }
