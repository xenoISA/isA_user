...
class AuthorizationService:
    ...
    async def check_resource_access(
        ...
        try:
            ...
            logger.debug(
                f"Checking access: user={user_id}, resource={resource_type}:{resource_name}, level={required_level}"
            )
    ...
    async def _check_organization_access(
        ...
        try:
            ...
            org_permission = await self.repository.get_organization_permission(
    ...
    async def _check_subscription_access(
        ...
        try:
            logger.info(
                f"Checking subscription access for user={user_id}, subscription={subscription_status}, resource={resource_type.value}:{resource_name}"
            )
            ...
            logger.info(
                f"Resource config found: subscription_required={resource_permission.subscription_tier_required.value}, access_level={resource_permission.access_level.value}"
            )

            # Check if user's subscription meets requirements
            user_tier = (
                SubscriptionTier(subscription_status)
                if subscription_status in [t.value for t in SubscriptionTier]
                else SubscriptionTier.FREE
            )
            logger.info(
                f"User tier: {user_tier.value}, Required tier: {resource_permission.subscription_tier_required.value}"
            )

            tier_sufficient = self._subscription_tier_sufficient(
                user_tier, resource_permission.subscription_tier_required
            )
    ...
    async def grant_resource_permission(self, request: GrantPermissionRequest) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Granted permission: user={request.user_id}, resource={request.resource_type}:{request.resource_name}, level={request.access_level}"
                )
                return True
    ...
    async def revoke_resource_permission(
        ...
        try:
            ...
            if result:
                        ...
                        try:
                            ...
                            event = Event(
                                ...
                                data={
                                    ...
                                    "resource_type": request.resource_type.value,
                                    "resource_name": request.resource_name,
                                    "previous_access_level": current_permission.access_level.value
                                    if current_permission
                                    else "none",
                ...
                logger.info(
                    f"Revoked permission: user={request.user_id}, resource={request.resource_type}:{request.resource_name}"
                )
                return True
    ...
    async def initialize_default_permissions(self) -> bool:
        ...
        try:
            ...
            for permission in default_permissions:
                existing = await self.repository.get_resource_permission(
    ...
    async def assign_role(self, request: AssignRoleRequest) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": (f"{assigner!r} may assign {normalized!r} at {scope} scope"),
        }
    ...
    def _organization_plan_sufficient(self, org_plan: str, required_plan: str) -> bool:
        """Check if organization plan meets requirements"""
        plan_hierarchy = {"startup": 0, "growth": 1, "enterprise": 2, "custom": 3}
    ...
    async def _log_permission_action(
        self,
        user_id: str,
        resource_type: ResourceType,
        resource_name: str,
