...
-- Insert test user permissions (using standardized user IDs)
...
VALUES
    ('user_permission', 'user', 'test_user_001', 'mcp_tool', 'filesystem_read', 'read_only', 'subscription', 'free', TRUE, '{}'::jsonb, NOW(), NOW()),
    ('user_permission', 'user', 'test_user_002', 'mcp_tool', 'filesystem_write', 'read_write', 'subscription', 'pro', TRUE, '{}'::jsonb, NOW(), NOW()),
    ('user_permission', 'user', 'test_user_003', 'ai_model', 'gpt4', 'read_write', 'subscription', 'enterprise', TRUE, '{}'::jsonb, NOW(), NOW())
ON CONFLICT DO NOTHING;

-- Insert test organization permissions (using standardized org IDs)
...
VALUES
    ('org_permission', 'organization', 'test_org_001', 'api_endpoint', 'user_api', 'admin', 'admin_grant', TRUE, '{"members": 10}'::jsonb, NOW(), NOW()),
    ('org_permission', 'organization', 'test_org_002', 'database', 'postgres_read', 'read_write', 'organization', TRUE, '{}'::jsonb, NOW(), NOW())
ON CONFLICT DO NOTHING;
