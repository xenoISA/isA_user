...
setup_metrics(app, "authorization_service")
...
async def service_stats():
    ...
    stats = {"total_permissions": 0, "active_users": 0, "resource_types": 0}
...
async def check_resource_access(request: ResourceAccessRequest):
    ...
    try:
        logger.info(
            f"Checking access: user={request.user_id}, resource={request.resource_type}:{request.resource_name}"
        )
...
async def grant_permission(request: GrantPermissionRequest):
    ...
    try:
        logger.info(
            f"Granting permission: user={request.user_id}, resource={request.resource_type}:{request.resource_name}"
        )
...
async def revoke_permission(request: RevokePermissionRequest):
    ...
    try:
        logger.info(
            f"Revoking permission: user={request.user_id}, resource={request.resource_type}:{request.resource_name}"
        )
...
@app.get(
    "/api/v1/authorization/user-permissions/{user_id}",
