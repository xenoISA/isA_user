...
SERVICE_PORT = (
    ...
    if config and config.service_port
    else int(os.getenv("COMPLIANCE_SERVICE_PORT", "8226"))
)
SERVICE_VERSION = "1.0.0"
SERVICE_HOST = (
    config.service_host
    if config and config.service_host
    else os.getenv("COMPLIANCE_SERVICE_HOST", "0.0.0.0")
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(
            f"[{SERVICE_NAME}] Service started successfully on port {SERVICE_PORT}"
        )
...
setup_metrics(app, "compliance_service")
...
def get_compliance_service() -> ComplianceService:
    ...
    if compliance_service is None:
        raise HTTPException(
            status_code=503, detail="Compliance service not initialized"
        )
...
def get_compliance_repository() -> ComplianceRepository:
    ...
    if compliance_repository is None:
        raise HTTPException(
            status_code=503, detail="Compliance repository not initialized"
        )
...
health = HealthCheck(
    "compliance_service", version="1.0.0", shutdown_manager=shutdown_manager
)
...
async def service_status(service: ComplianceService = Depends(get_compliance_service)):
    """服务状态详情"""
    ...
    try:
        ...
        for item in request.items:
            check_req = ComplianceCheckRequest(
                user_id=request.user_id, organization_id=request.organization_id, **item
            )
        ...
        return BatchComplianceCheckResponse(
            ...
            summary={
                ...
                "avg_processing_time": sum(r.processing_time_ms for r in results)
                / len(results)
                if results
                else 0,
            },
...
async def get_check_by_id(
    check_id: str, repo: ComplianceRepository = Depends(get_compliance_repository)
):
    """获取特定合规检查记录"""
    try:
    ...
    try:
        ...
        if request.include_violations:
            violations = await repo.get_checks_by_organization(
                organization_id=request.organization_id, limit=1000
            )
...
async def get_policy(
    policy_id: str, repo: ComplianceRepository = Depends(get_compliance_repository)
):
    """获取策略详情"""
    try:
...
async def get_statistics(
    ...
    try:
        ...
        return ComplianceStats(
            ...
            violations_today=stats_today.get("failed_checks", 0)
            + stats_today.get("flagged_checks", 0),
            violations_7d=stats_7d.get("failed_checks", 0)
            + stats_7d.get("flagged_checks", 0),
            violations_30d=stats_30d.get("failed_checks", 0)
            + stats_30d.get("flagged_checks", 0),
...
async def export_user_data(
    ...
    **Example:**
    ...
    try:
        ...
        checks = await repo.get_checks_by_user(
            user_id=user_id, limit=10000  # Get all records
        )
        ...
        if format == "json":
            ...
            for check in export_data["checks"]:
                writer.writerow(
                    {k: v for k, v in check.items() if k in writer.fieldnames}
                )
...
async def get_user_data_summary(
    user_id: str, repo: ComplianceRepository = Depends(get_compliance_repository)
):
    """
...
async def update_user_consent(
    ...
    consent_type: str = Query(
        ..., description="Type: data_processing, marketing, analytics"
    ),
    ...
    **Example:**
    ...
    try:
        ...
        timestamp = datetime.utcnow().isoformat()
        logger.info(
            f"Consent {'granted' if granted else 'revoked'} for {user_id}: {consent_type}"
        )
