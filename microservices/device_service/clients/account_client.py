...
Client for device_service to interact with account_service.
Used for validating user ownership of devices.
"""
...
class AccountClient:
    """
    Wrapper client for Account Service calls from Device Service.

    This wrapper provides device-specific convenience methods
    ...
    async def validate_user_exists(self, user_id: str) -> bool:
        ...
        Used before registering devices for users.
    ...
    async def get_user_info(self, user_id: str) -> Optional[dict]:
        """
        Get user information for device context.
    ...
    async def check_user_active(self, user_id: str) -> bool:
        ...
        Prevents device registration for suspended/deleted accounts.
    ...
    async def get_user_subscription_tier(self, user_id: str) -> Optional[str]:
        """
        Get user subscription tier for device limits.

        Different subscription tiers may allow different numbers of devices.
        ...
        try:
            profile = await self._client.get_account_profile(user_id)
            if profile:
                return profile.get("subscription_status") or profile.get("subscription_plan")
            return None
        except Exception:
            return None
