...
class DeviceOnlineEventData(BaseModel):
    ...
    offline_duration_seconds: Optional[int] = Field(
        None, description="How long device was offline"
    )
...
def create_device_registered_event_data(
    device_id: str, device_name: str, device_type: str, owner_id: Optional[str] = None
...
def create_device_status_changed_event_data(
    device_id: str, old_status: str, new_status: str, reason: Optional[str] = None
) -> DeviceStatusChangedEventData:
    """Create device status changed event data"""
    return DeviceStatusChangedEventData(
        device_id=device_id, old_status=old_status, new_status=new_status, reason=reason
    )
...
def create_device_firmware_updated_event_data(
    device_id: str, old_version: str, new_version: str, update_id: Optional[str] = None
