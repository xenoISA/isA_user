...
class SubscriptionTier(BaseModel):
    ...
    monthly_credits: int = Field(
        default=0, ge=0, description="Credits included per month"
    )
    credit_rollover: bool = Field(
        default=False, description="Whether unused credits roll over"
    )
    max_rollover_credits: Optional[int] = Field(
        default=None, description="Max credits that can roll over"
    )
...
class CostDefinition(BaseModel):
    ...
    service_type: str = Field(
        ..., description="Service type (model_inference, storage_minio, etc.)"
    )
    ...
    unit_size: int = Field(
        default=1, ge=1, description="Units per charge (e.g., 1000 for per 1K tokens)"
    )
