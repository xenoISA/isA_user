...
from .admin_audit_models import (
    AdminAuditCreateRequest,
    AdminAuditResponse,
    AdminAuditQueryResponse,
    AdminAuditLogEntry,
)
...
health = HealthCheck(
    "audit_service", version="1.0.0", shutdown_manager=shutdown_manager
)
health.add_check(
    "postgres",
    lambda: audit_service.repository.check_connection()
    if audit_service and audit_service.repository
    else False,
)
...
async def log_audit_event(
    request: AuditEventCreateRequest, svc: AuditService = Depends(get_audit_service)
):
    """记录审计事件"""
    try:
...
async def query_audit_events(
    query: AuditQueryRequest, svc: AuditService = Depends(get_audit_service)
):
    """查询审计事件"""
    try:
...
async def create_security_alert(
    alert: SecurityAlertRequest, svc: AuditService = Depends(get_audit_service)
):
    """创建安全告警"""
    try:
...
async def generate_compliance_report(
    request: ComplianceReportRequest, svc: AuditService = Depends(get_audit_service)
):
    """生成合规报告"""
    try:
...
def get_admin_audit_repo() -> AdminAuditRepository:
    ...
    if not admin_audit_repo:
        raise HTTPException(
            status_code=503, detail="Admin audit repository not available"
        )
...
async def get_admin_actions(
    ...
    action: Optional[str] = Query(
        None, description="Filter by action (e.g. create_product)"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to query admin audit log: {str(e)}"
        )


@app.post(
    "/api/v1/audit/admin/actions", response_model=AdminAuditResponse, status_code=201
)
async def record_admin_action(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to record admin action: {str(e)}"
        )
