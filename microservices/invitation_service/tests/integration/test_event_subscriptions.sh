...
if [ -n "$HANDLER_LOGS" ]; then
    ...
    PASSED_1=0
    if echo "$HANDLER_LOGS" | grep -q "organization.deleted"; then
        echo -e "${GREEN}  ✓ organization.deleted handler registered${NC}"
        PASSED_1=$((PASSED_1 + 1))
    else
        echo -e "${RED}  ✗ organization.deleted handler NOT registered${NC}"
    fi
