"""Observability hooks for memory_service upstream-fallback paths.

Mirrors ``microservices/artifact_service/observability.py`` — see that module
for the full rationale.  We keep two separate files (one per microservice)
because the Prometheus counter must be a module-level singleton and we want
each service to expose its own metric prefix:

    * ``artifact_service_upstream_fallback_total``
    * ``memory_service_upstream_fallback_total``

Co-locating Sentry init in the same module keeps the wiring simple — both
helpers are imported from ``main.py``'s lifespan.

Dashboards (deferred — see PR description for #461):
  * Grafana panel: ``sum by (operation, reason) (rate(memory_service_upstream_fallback_total[5m]))``
  * Sentry: filter ``service:memory_service`` + tag ``operation``.
"""
...
logger = logging.getLogger("microservices.memory_service.observability")
...
try:  # pragma: no cover - exercised in production
    ...
    upstream_fallback_total = create_counter(
        "memory_service_upstream_fallback_total",
        "Number of times an upstream LLM call failed and the service fell back to a deterministic stub.",
        ["operation", "reason"],
    )
...
def init_sentry(service_name: str = "memory_service") -> bool:
...
def classify_reason(exc: BaseException) -> str:
    """Bucket the exception into a low-cardinality ``reason`` label."""
...
def record_upstream_fallback(
    *,
    ...
    """Increment the fallback counter, send to Sentry, and emit a WARN log."""
