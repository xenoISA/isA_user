...
Return ONLY valid JSON with an "episodes" array."""
    ...
    async def vector_search(
        ...
        """
        Search episodic memories using vector similarity (Qdrant)
        ...
        try:
            ...
            logger.info(f"Generated query embedding with {len(query_embedding)} dimensions for episodic search: {query[:50]}...")
            ...
            logger.error(f"Episodic vector search failed: {e}")
            return await self.search_episodes_by_event_type(user_id, query, limit)
