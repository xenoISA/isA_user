...
class MemoryService:
    ...
    async def create_memory(self, request: MemoryCreateRequest) -> MemoryOperationResult:
        """
        Create a new memory
        ...
        try:
            ...
            if request.memory_type == MemoryType.SESSION:
                ...
                memory_data["session_id"] = getattr(request, "session_id", None) or request.context.get(
            ...
            if result:
                ...
                return MemoryOperationResult(success=False, operation="create", message="Failed to create memory")

        except Exception as e:
            logger.error(f"Error creating memory: {e}")
            return MemoryOperationResult(success=False, operation="create", message=f"Error: {str(e)}")
    ...
    async def list_memories(self, params: MemoryListParams) -> List[Dict[str, Any]]:
        ...
        try:
            if params.memory_type:
                ...
                # Sort by created_at
                all_memories.sort(key=lambda x: x.get("created_at", datetime.min), reverse=True)
    ...
    async def delete_memory(self, memory_id: str, memory_type: MemoryType, user_id: str) -> MemoryOperationResult:
        """
        Delete a memory
        ...
        try:
            ...
            for memory_id in memory_ids:
                ...
                if candidates:
                    ...
                    logger.info(f"A-MEM: linked {result['created_count']} associations for {memory_type}/{memory_id}")
    ...
    def _schedule_link_associations(
        ...
        """
        task = asyncio.create_task(self._link_associations(memory_ids, memory_type, user_id))
    ...
    async def store_factual_memory(
        ...
        summary_service.synthesize_summary (PR #454).
        """
    ...
    async def search_episodes_by_timeframe(
        ...
        return await self.episodic_service.repository.search_by_timeframe(user_id, start_date, end_date, limit)
    ...
    async def deactivate_session(self, user_id: str, session_id: str) -> MemoryOperationResult:
