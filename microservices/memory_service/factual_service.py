        ...
        try:
            ...
            for fact in facts_data.get("facts", []):
                if self._is_valid_fact(fact):
                    ...
                    if existing:
                        logger.info(
                            f"Duplicate fact found, skipping: {fact.get('subject')} {fact.get('predicate')} {object_value}"
                        )
                        continue
                    ...
                    if result:
                        ...
                        if embedding:
                            try:
                                async with self.qdrant:
                                    upsert_result = await self.qdrant.upsert_points(
                                        "factual_memories",
                                        [
                                            {
                                                "id": memory_id,
                                ...
                                logger.info(
                                    f"Stored embedding to Qdrant for memory {memory_id}, user_id={user_id}, result={upsert_result}"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Failed to store embedding to Qdrant: {e}",
                                    exc_info=True,
                                )
                        else:
...
{"facts": [{"fact_type": "person", "subject": "John", "predicate": "lives in", "object_value": "Tokyo", "confidence": 0.9, "context": "mentioned in conversation"}]}"""
    ...
    def _create_fact_content(self, fact: Dict[str, Any]) -> str:
        ...
        if fact_type == "person" and predicate in [
            "is named",
            "name is",
            "is called",
            "named",
            ...
            async with self.qdrant:
                ...
                try:
                    ...
                    if unfiltered_results:
                        logger.info(
                            f"Unfiltered search found {len(unfiltered_results)} results, top scores: {[r.get('score') for r in unfiltered_results[:3]]}"
                        )
                        # Log user_ids in results
                        for r in unfiltered_results[:3]:
                            payload = r.get("payload", {})
                            logger.info(
                                f"  - Point {r.get('id')}: user_id={payload.get('user_id')}, score={r.get('score')}"
                            )
                    else:
                        logger.warning(
                            "Unfiltered search returned no results - collection may be empty or embeddings not matching"
                        )
                except Exception as e:
