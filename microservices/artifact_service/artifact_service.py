...
class ArtifactService:
    ...
    async def runtime_invoke(
        ...
        try:
            ...
            # #461: surface the fallback to Sentry + Prometheus so silent
            # degradations show up on the dashboard instead of getting buried
            # in WARN logs.  The helper also re-logs at WARN with structured
            # fields, but that's fine — duplicate signals are cheaper than
            # missing ones, and the existing WARN above still anchors local
            # log greps to the original wording.
    ...
    async def mcp_call(
        ...
        try:
            ...
            # #461: same observability hook as runtime_invoke — the MCP path
            # has its own ``operation`` label so Grafana can split the two
            # upstream dependencies (isA_Model vs isA_MCP).
            from .observability import record_upstream_fallback
