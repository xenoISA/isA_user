...
class OTAService:
    ...
    async def upload_firmware(
        self, user_id: str, firmware_data: Dict[str, Any], file_content: bytes
    ) -> Optional[FirmwareResponse]:
        """上传固件文件"""
        try:
            # 验证文件大小
            ...
            if self.storage_client:
                try:
                    ...
                    if storage_result and storage_result.get("download_url"):
                        ...
                        logger.warning(
                            "Failed to upload to storage service, using local URL"
                        )
                except Exception as storage_error:
                    logger.warning(
                        f"Storage service error: {storage_error}, continuing with local storage"
                    )
            else:
                logger.warning(
                    "Storage client not initialized, using local storage URL"
                )
            ...
            firmware = FirmwareResponse(
                ...
                success_rate=float(db_result["success_rate"])
                if db_result.get("success_rate")
                else 0.0,
                created_at=datetime.fromisoformat(
                    db_result["created_at"].replace("Z", "+00:00")
                )
                if isinstance(db_result["created_at"], str)
                else db_result["created_at"],
                updated_at=datetime.fromisoformat(
                    db_result["updated_at"].replace("Z", "+00:00")
                )
                if isinstance(db_result["updated_at"], str)
                else db_result["updated_at"],
    ...
    async def create_update_campaign(
        self, user_id: str, campaign_data: Dict[str, Any]
    ) -> Optional[UpdateCampaignResponse]:
        """创建更新活动"""
        try:
            ...
            campaign_db_data = {
                ...
                "deployment_strategy": campaign_data.get(
                    "deployment_strategy", DeploymentStrategy.STAGED
                ).value
                if hasattr(
                    campaign_data.get("deployment_strategy", DeploymentStrategy.STAGED),
                    "value",
                )
                else str(
                    campaign_data.get("deployment_strategy", DeploymentStrategy.STAGED)
                ),
                "priority": campaign_data.get("priority", Priority.NORMAL).value
                if hasattr(campaign_data.get("priority", Priority.NORMAL), "value")
                else str(campaign_data.get("priority", Priority.NORMAL)),
                ...
                "max_concurrent_updates": campaign_data.get(
                ...
                "failure_threshold_percent": campaign_data.get(
                    "failure_threshold_percent", 20
                ),
                "rollback_triggers": campaign_data.get(
                    "rollback_triggers", ["failure_rate"]
                ),
            ...
            target_devices_list = db_result.get("target_devices") or campaign_data.get(
                "target_devices", []
            )
            campaign = UpdateCampaignResponse(
                ...
                deployment_strategy=DeploymentStrategy(
                    db_result.get("deployment_strategy", "staged")
                ),
                priority=Priority(db_result.get("priority", "normal")),
                target_device_count=len(
                    target_devices_list
                ),  # Calculate from target_devices
                ...
                scheduled_start=datetime.fromisoformat(
                    db_result["start_time"].replace("Z", "+00:00")
                )
                if db_result.get("start_time")
                and isinstance(db_result["start_time"], str)
                else db_result.get("start_time"),
                scheduled_end=datetime.fromisoformat(
                    db_result["end_time"].replace("Z", "+00:00")
                )
                if db_result.get("end_time") and isinstance(db_result["end_time"], str)
                else db_result.get("end_time"),
                actual_start=None,
                actual_end=None,
                timeout_minutes=campaign_data.get("timeout_minutes", 60),  # Not in DB
                auto_rollback=db_result.get("auto_rollback", True),
                failure_threshold_percent=db_result.get(
                    "rollback_threshold", 10.0
                ),  # DB uses rollback_threshold
                rollback_triggers=campaign_data.get(
                    "rollback_triggers", []
                ),  # Not in DB
                requires_approval=campaign_data.get(
    ...
    async def update_single_device(
        self, device_id: str, update_data: Dict[str, Any]
    ) -> Optional[DeviceUpdateResponse]:
        """更新单个设备"""
        try:
            ...
            if self.device_client:
                try:
                    ...
                    if not device:
                        raise ValueError(
                            f"Device '{device_id}' not found. Device must be registered in Device Service first."
                        )

                    # Get current firmware version
                    from_version = await self.device_client.get_device_firmware_version(
                        device_id
                    )

                    # Optional: Check firmware compatibility
                    is_compatible = (
                        await self.device_client.check_firmware_compatibility(
                            device_id,
                    ...
                    if not is_compatible:
                        logger.warning(
                            f"Firmware may not be compatible with device {device_id}"
                        )
                    ...
                    logger.warning(
                        "Proceeding with update without device validation (Device Service unavailable)"
                    )
            else:
                logger.warning(
                    "Device client not initialized, skipping device validation"
                )

            # Prepare data for database
            timeout_at = datetime.utcnow() + timedelta(
                minutes=update_data.get("timeout_minutes", 60)
            )
            ...
            db_result = await self.repository.create_device_update(
                device_update_db_data
            )
            if not db_result:
            ...
            logger.info(
                f"Device update created and saved: {update_id} for device {device_id}"
            )
    ...
    async def rollback_update(
        self, device_id: str, to_version: str
    ) -> Optional[RollbackResponse]:
        """回滚更新"""
        try:
    ...
    async def get_update_stats(self) -> Optional[UpdateStatsResponse]:
        ...
        try:
            ...
            last_24h_data_transferred = min(
                total_data_transferred, 5 * 1024 * 1024 * 1024
            )  # Simplified for now
    ...
    async def get_campaign(self, campaign_id: str) -> Optional[UpdateCampaignResponse]:
        ...
        try:
            ...
            return UpdateCampaignResponse(
                ...
                status=UpdateStatus(data["status"]),
                deployment_strategy=DeploymentStrategy(
                    data.get("deployment_strategy", "staged")
                ),
    ...
    async def _calculate_target_devices(
        self, device_ids: List[str], group_ids: List[str], filters: Dict[str, Any]
    ) -> int:
        """计算目标设备数量"""
        # 实际应该查询设备管理服务
