...
async def upload_firmware(
    ...
    try:
        ...
        if not any(
            ...
            for ext in [".bin", ".hex", ".elf", ".tar.gz", ".zip"]
        ):
        ...
        firmware = await microservice.service.upload_firmware(
            user_context["user_id"], firmware_data, file_content
        )
...
async def list_firmware(
    ...
    is_security_update: Optional[bool] = Query(
        None, description="Filter security updates"
    ),
            ...
            firmware_responses.append(
                FirmwareResponse(
                    ...
                    created_at=datetime.fromisoformat(
                        fw["created_at"].replace("Z", "+00:00")
                    )
                    if isinstance(fw["created_at"], str)
                    else fw["created_at"],
                    updated_at=datetime.fromisoformat(
                        fw["updated_at"].replace("Z", "+00:00")
                    )
                    if isinstance(fw["updated_at"], str)
                    else fw["updated_at"],
            ...
            campaign_responses.append(
                UpdateCampaignResponse(
                    ...
                    scheduled_start=datetime.fromisoformat(
                        camp["scheduled_start"].replace("Z", "+00:00")
                    )
                    if camp.get("scheduled_start")
                    and isinstance(camp["scheduled_start"], str)
                    else camp.get("scheduled_start"),
                    scheduled_end=datetime.fromisoformat(
                        camp["scheduled_end"].replace("Z", "+00:00")
                    )
                    if camp.get("scheduled_end")
                    and isinstance(camp["scheduled_end"], str)
                    else camp.get("scheduled_end"),
                    actual_start=datetime.fromisoformat(
                        camp["actual_start"].replace("Z", "+00:00")
                    )
                    if camp.get("actual_start")
                    and isinstance(camp["actual_start"], str)
                    else camp.get("actual_start"),
                    actual_end=datetime.fromisoformat(
                        camp["actual_end"].replace("Z", "+00:00")
                    )
                    if camp.get("actual_end") and isinstance(camp["actual_end"], str)
                    else camp.get("actual_end"),
                    ...
                    created_at=datetime.fromisoformat(
                        camp["created_at"].replace("Z", "+00:00")
                    )
                    if isinstance(camp["created_at"], str)
                    else camp["created_at"],
                    updated_at=datetime.fromisoformat(
                        camp["updated_at"].replace("Z", "+00:00")
                    )
                    if isinstance(camp["updated_at"], str)
                    else camp["updated_at"],
...
@app.post(
    "/api/v1/ota/campaigns/{campaign_id}/approve", response_model=UpdateCampaignResponse
)
...
@app.get(
    "/api/v1/ota/devices/{device_id}/updates", response_model=UpdateHistoryResponse
)
...
async def get_update_stats(user_context: Dict[str, Any] = Depends(get_user_context)):
    """获取更新统计"""
    try:
...
async def bulk_update_devices(
    ...
    for device_id in device_ids:
        try:
            update_data = {"firmware_id": firmware_id, "priority": priority}
            device_update = await microservice.service.update_single_device(
