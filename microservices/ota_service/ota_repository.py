...
class OTARepository:
    ...
    async def create_firmware(
        ...
        try:
            ...
            duplicate = await self.get_firmware_by_model_version(
                firmware_data["device_model"], firmware_data["version"]
            )
            if duplicate:
                logger.info(
                    f"Firmware version already exists for {firmware_data['device_model']} v{firmware_data['version']}"
                )
                return duplicate
        ...
        try:
            ...
            async with self.db:
                results = await self.db.query(
                    query, [device_model, version], schema=self.schema
                )
    ...
    async def create_campaign(
        ...
        try:
            ...
            # Map time fields
            start_time = campaign_data.get("scheduled_start") or campaign_data.get(
                "start_time"
            )
            end_time = campaign_data.get("scheduled_end") or campaign_data.get(
                "end_time"
            )
            ...
            params = [
                ...
                campaign_data.get("rollback_threshold")
                or campaign_data.get("failure_threshold_percent", 10.0),
    ...
    async def update_campaign_status(
        self, campaign_id: str, status: str, **kwargs
    ) -> bool:
    ...
    async def create_device_update(
    ...
    async def get_update_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.db:
                campaign_results = await self.db.query(
                    campaign_query, [], schema=self.schema
                )
                update_results = await self.db.query(
                    update_query, [], schema=self.schema
                )
            ...
            total = (update_data.get("completed", 0) or 0) + (
                update_data.get("failed", 0) or 0
            )
            success_rate = (
                ((update_data.get("completed", 0) or 0) / total * 100)
                if total > 0
                else 0.0
            )
