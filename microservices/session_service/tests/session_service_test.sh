...
print_section "Test 3: Create Session"
...
# Test 4: Get Session
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 4: Get Session"
...
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 5: Add User Message"
...
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 6: Add Assistant Message"
    echo "POST ${API_BASE}/sessions/${SESSION_ID}/messages"
...
EOF
    ...
    echo -e "${YELLOW}Skipping Test 6: No session available${NC}"
    ((TESTS_FAILED++))
fi

# Test 7: Get Session Messages
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 7: Get Session Messages"
...
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 8: Update Session"
...
# Test 12: Get User Sessions
print_section "Test 10: Get User Sessions"
echo "GET ${API_BASE}/users/${TEST_USER_ID}/sessions"

USER_SESSIONS_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "${API_BASE}/users/${TEST_USER_ID}/sessions?active_only=false&page=1&page_size=50")
...
print_section "Test 11: Get Service Statistics"
...
# Test 14: End Session
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 12: End Session"
    ...
    echo -e "${YELLOW}Skipping Test 14: No session available${NC}"
    ((TESTS_FAILED++))
fi

# Test 15: Verify Session is Ended
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    print_section "Test 13: Verify Session is Ended"
    echo "GET ${API_BASE}/sessions/${SESSION_ID}"
    ...
    echo -e "${YELLOW}Skipping Test 15: No session available${NC}"
    ((TESTS_FAILED++))
fi
