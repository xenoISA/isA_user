...
# Configuration
API_BASE="http://localhost/api/v1/telemetry"
...
METRIC_RESPONSE=$(curl -s -X POST "${API_BASE}/metrics" \
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/metrics" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$METRIC_REQUEST")
...
# Test 3: List Metrics
# ======================
print_section "Test 3: List Metric Definitions"
increment_test

echo "GET ${API_BASE}/metrics"
METRICS_LIST_RESPONSE=$(curl -s "${API_BASE}/metrics" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$METRICS_LIST_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/metrics" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 4: Get Metric Definition
# ======================
print_section "Test 4: Get Metric Definition"
increment_test

echo "GET ${API_BASE}/metrics/${METRIC_NAME}"
METRIC_DETAILS_RESPONSE=$(curl -s "${API_BASE}/metrics/${METRIC_NAME}" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$METRIC_DETAILS_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/metrics/${METRIC_NAME}" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 5: Ingest Single Data Point
# ======================
print_section "Test 5: Ingest Single Data Point"
increment_test

echo "POST ${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry"
DATA_POINT='{
  "metric_name": "temperature",
  "value": 25.5,
  "unit": "celsius",
...
INGEST_RESPONSE=$(curl -s -X POST "${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$DATA_POINT")
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$DATA_POINT")
...
# ======================
# Test 6: Ingest Batch Data Points
# ======================
print_section "Test 6: Ingest Batch Data Points"
increment_test

echo "POST ${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry/batch"
BATCH_DATA='{
  "data_points": [
    {
      "metric_name": "temperature",
...
BATCH_INGEST_RESPONSE=$(curl -s -X POST "${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry/batch" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$BATCH_DATA")
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/devices/${TEST_DEVICE_ID}/telemetry/batch" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$BATCH_DATA")
...
# ======================
# Test 7: Get Latest Value
# ======================
print_section "Test 7: Get Latest Value"
increment_test

echo "GET ${API_BASE}/devices/${TEST_DEVICE_ID}/metrics/${METRIC_NAME}/latest"
LATEST_RESPONSE=$(curl -s "${API_BASE}/devices/${TEST_DEVICE_ID}/metrics/${METRIC_NAME}/latest" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$LATEST_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/devices/${TEST_DEVICE_ID}/metrics/${METRIC_NAME}/latest" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 8: Get Device Metrics
# ======================
print_section "Test 8: Get Device Metrics"
increment_test

echo "GET ${API_BASE}/devices/${TEST_DEVICE_ID}/metrics"
DEVICE_METRICS_RESPONSE=$(curl -s "${API_BASE}/devices/${TEST_DEVICE_ID}/metrics" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$DEVICE_METRICS_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/devices/${TEST_DEVICE_ID}/metrics" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 9: Query Telemetry Data
# ======================
print_section "Test 9: Query Telemetry Data"
increment_test

echo "POST ${API_BASE}/query"
QUERY_REQUEST='{
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/query" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$QUERY_REQUEST")
...
# ======================
# Test 10: Create Alert Rule
# ======================
print_section "Test 10: Create Alert Rule"
increment_test

echo "POST ${API_BASE}/alerts/rules"
ALERT_RULE_REQUEST='{
...
ALERT_RULE_RESPONSE=$(curl -s -X POST "${API_BASE}/alerts/rules" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/alerts/rules" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$ALERT_RULE_REQUEST")
...
# ======================
# Test 11: List Alert Rules
# ======================
print_section "Test 11: List Alert Rules"
increment_test

echo "GET ${API_BASE}/alerts/rules"
ALERT_RULES_LIST_RESPONSE=$(curl -s "${API_BASE}/alerts/rules" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$ALERT_RULES_LIST_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/alerts/rules" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 12: Get Alert Rule Details
# ======================
if [ -n "$ALERT_RULE_ID" ] && [ "$ALERT_RULE_ID" != "unknown" ]; then
    print_section "Test 12: Get Alert Rule Details"
    increment_test

    echo "GET ${API_BASE}/alerts/rules/${ALERT_RULE_ID}"
    ALERT_RULE_DETAILS_RESPONSE=$(curl -s "${API_BASE}/alerts/rules/${ALERT_RULE_ID}" \
      -H "Authorization: Bearer ${TEST_TOKEN}")
    echo "Response:"
    echo "$ALERT_RULE_DETAILS_RESPONSE" | jq '.'
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/alerts/rules/${ALERT_RULE_ID}" \
      -H "Authorization: Bearer ${TEST_TOKEN}")
    echo "HTTP Status: $HTTP_CODE"
...
# Test 13: List Alerts
# ======================
print_section "Test 13: List Alerts"
increment_test

echo "GET ${API_BASE}/alerts"
ALERTS_LIST_RESPONSE=$(curl -s "${API_BASE}/alerts" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$ALERTS_LIST_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/alerts" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 14: Get Device Stats
# ======================
print_section "Test 14: Get Device Telemetry Statistics"
increment_test

echo "GET ${API_BASE}/devices/${TEST_DEVICE_ID}/stats"
DEVICE_STATS_RESPONSE=$(curl -s "${API_BASE}/devices/${TEST_DEVICE_ID}/stats" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
echo "$DEVICE_STATS_RESPONSE" | jq '.'
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${API_BASE}/devices/${TEST_DEVICE_ID}/stats" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "HTTP Status: $HTTP_CODE"
...
# ======================
# Test 15: Get Service Telemetry Stats
# ======================
print_section "Test 15: Get Service Telemetry Statistics"
increment_test

echo "GET ${API_BASE}/stats"
TELEMETRY_STATS_RESPONSE=$(curl -s "${API_BASE}/stats" \
  -H "Authorization: Bearer ${TEST_TOKEN}")
echo "Response:"
...
# ======================
# Test 16: Create Real-time Subscription
# ======================
print_section "Test 16: Create Real-time Subscription"
increment_test

echo "POST ${API_BASE}/subscribe"
...
SUBSCRIPTION_RESPONSE=$(curl -s -X POST "${API_BASE}/subscribe" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API_BASE}/subscribe" \
  -H "Authorization: Bearer ${TEST_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "$SUBSCRIPTION_REQUEST")
