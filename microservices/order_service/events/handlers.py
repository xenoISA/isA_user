...
async def handle_payment_completed(
    event_data: Dict[str, Any], order_service, event_id: str = None
) -> None:
    """
    ...
    try:
        ...
        order = await order_service.repository.get_order_by_payment_intent(
            payment_intent_id
        )
        if not order:
        ...
        complete_request = OrderCompleteRequest(
            ...
            transaction_id=event_data.get("payment_id")
        ...
        if order_service.inventory_client and order_service.fulfillment_client:
            ...
            if shippable_items and order.shipping_address:
                address = (
                    ...
                    if hasattr(order.shipping_address, "model_dump")
                ...
                shipment = await order_service.fulfillment_client.create_shipment(
                    order_id=order.order_id, items=shippable_items, address=address
                )
...
async def handle_payment_failed(
    ...
    try:
        ...
        await order_service.repository.update_order_status(
            order.order_id, OrderStatus.FAILED, payment_status=PaymentStatus.FAILED
        )
...
async def handle_payment_refunded(
    ...
    try:
        ...
        if not payment_id and not payment_intent_id:
            logger.warning(
                "payment.refunded event missing payment_id and payment_intent_id"
            )
            return

        logger.info(
            f"Processing payment refund for payment {payment_id or payment_intent_id}, amount: {refund_amount}"
        )
        ...
        if not order and payment_id:
            ...
            order = await order_service.repository.get_order_by_transaction_id(
                payment_id
            )

        if not order:
            logger.warning(
                f"No order found for payment {payment_id or payment_intent_id}"
            )
            if event_id:
                mark_event_processed(event_id)
            return
        ...
        await order_service.repository.update_order_status(
            order.order_id, OrderStatus.REFUNDED, payment_status=PaymentStatus.REFUNDED
        )
        ...
        logger.info(
            f"✅ Order {order.order_id} marked as refunded (amount: {refund_amount})"
        )
...
async def handle_wallet_credits_added(
    event_data: Dict[str, Any], order_service, event_id: str = None
) -> None:
    """
    ...
    try:
        ...
        pending_orders = await order_service.repository.get_pending_orders_by_user(
            ...
            if hasattr(OrderType, "CREDIT_PURCHASE")
            else None,
        )
        ...
        for order in pending_orders:
            ...
            if reference_id and str(order.order_id) == reference_id:
                ...
                fulfilled_count += 1
                logger.info(
                    f"✅ Fulfilled order {order.order_id} after wallet credits added"
                )
        ...
        if fulfilled_count > 0:
            logger.info(
                f"Fulfilled {fulfilled_count} pending orders for user {user_id}"
            )

    except Exception as e:
        logger.error(
            f"❌ Failed to handle wallet.credits_added event: {e}", exc_info=True
        )
...
async def handle_subscription_created(
    ...
    try:
        ...
        logger.info(
            f"Subscription created: {subscription_id} for user {user_id}, plan: {plan_id}"
        )
        ...
        order_request = OrderCreateRequest(
            ...
            if hasattr(OrderType, "SUBSCRIPTION")
            else OrderType.SERVICE,
        ...
        complete_request = OrderCompleteRequest(
            payment_confirmed=True, transaction_id=subscription_id
        )
        await order_service.complete_order(order.order_id, complete_request)
        ...
        logger.info(
            f"✅ Created subscription order {order.order_id} for subscription {subscription_id}"
        )

    except Exception as e:
        logger.error(
            f"❌ Failed to handle subscription.created event: {e}", exc_info=True
        )
...
async def handle_subscription_canceled(
    ...
    try:
        ...
        logger.info(
            f"✅ Cancelled {cancelled_count} pending orders for subscription {subscription_id}"
        )

    except Exception as e:
        logger.error(
            f"❌ Failed to handle subscription.canceled event: {e}", exc_info=True
        )
