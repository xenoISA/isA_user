...
class OrderService:
        ...
        try:
            ...
            try:
                async with self.account_client as client:
                    ...
                    if not user_account:
                        logger.warning(
                            f"User {request.user_id} not found in Account Service - proceeding anyway (dev/test mode)"
                        )
                    else:
                        logger.info(
                            f"User {request.user_id} validated via Account Service for order creation"
                        )
            except Exception as e:
                logger.warning(f"Failed to validate user via Account Service: {e}")
                logger.info(
                    "Proceeding with order creation despite Account Service validation failure"
                )
            ...
            if subtotal_amount is None or final_amount is None:
                computed_subtotal, computed_final = self._calculate_totals(
                    items_payload
                )
                subtotal_amount = (
                    subtotal_amount
                    if subtotal_amount is not None
                    else computed_subtotal
                )
                final_amount = (
                    final_amount if final_amount is not None else computed_final
                )
            ...
            order = await self.order_repo.create_order(
                ...
                billing_address=request.billing_address.model_dump(mode="json")
            ...
            if self.inventory_client:
                shippable_items = [
                    i for i in items_payload if i.get("fulfillment_type") == "ship"
                ]
                if shippable_items:
                    reservation = await self.inventory_client.reserve(
                        order.order_id, shippable_items
                    )
                    if not reservation:
            ...
            if self.payment_client:
                ...
                if payment_response and payment_response.get("payment_intent_id"):
                    ...
                    await self.order_repo.update_order(
                        order_id=order.order_id, payment_intent_id=payment_intent_id
                    )
            ...
            if self.event_bus:
                try:
                    ...
                    if self._publish_order_created:
                        ...
                        logger.info(
                            f"Published order.created event for order {order.order_id}"
                        )
                except Exception as e:
            ...
            return OrderResponse(
                success=True, order=order, message="Order created successfully"
            )
    ...
    async def update_order(
        self, order_id: str, request: OrderUpdateRequest
    ) -> OrderResponse:
        """Update order"""
        try:
            # Get existing order
    ...
    async def cancel_order(
        self, order_id: str, request: OrderCancelRequest
    ) -> OrderResponse:
        """Cancel an order"""
        try:
            # Get existing order
                ...
                if self.event_bus:
                    try:
                        ...
                        if self._publish_order_canceled:
                            ...
                            logger.info(
                                f"Published order.canceled event for order {order_id}"
                            )
                    except Exception as e:
    ...
    async def complete_order(
        self, order_id: str, request: OrderCompleteRequest
    ) -> OrderResponse:
        ...
        try:
            ...
            success = await self.order_repo.complete_order(
                order_id=order_id, payment_intent_id=request.transaction_id
            )

            if success:
                # If this is a credit purchase, add credits to wallet
                if (
                    existing_order.order_type == OrderType.CREDIT_PURCHASE
                    and existing_order.wallet_id
                    and request.credits_added
                ):
                    ...
                    try:
                        ...
                        if self._publish_order_completed:
                            await self._publish_order_completed(
                                ...
                                if request.credits_added
                                else 0,
                            )
                            logger.info(
                                f"Published order.completed event for order {order_id}"
                            )
                    except Exception as e:
    ...
    async def search_orders(self, params: OrderSearchParams) -> List[Order]:
        ...
        try:
            return await self.order_repo.search_orders(
                query=params.query, limit=params.limit, user_id=params.user_id
            )
        except Exception as e:
    ...
    async def _add_credits_to_wallet(
        self, user_id: str, wallet_id: str, amount: Decimal, order_id: str
    ) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Wallet service error: {response.status_code} - {response.text}"
                    )
                    return False
    ...
    def _validate_order_create_request(self, request: OrderCreateRequest) -> None:
        ...
        if request.order_type == OrderType.SUBSCRIPTION and not request.subscription_id:
            raise OrderValidationError(
                "subscription_id is required for subscription orders"
            )

        # Require shipping address if any item needs shipping
        if request.items:
            requires_shipping = any(
                item.fulfillment_type.value == "ship" for item in request.items
            )
            if requires_shipping and not request.shipping_address:
                raise OrderValidationError(
                    "shipping_address is required for shippable items"
                )
