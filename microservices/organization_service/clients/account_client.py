...
Client for organization_service to interact with account_service.
Used for validating user existence before organization operations.
"""
...
class AccountClient:
    """
    Wrapper client for Account Service calls from Organization Service.

    This wrapper provides organization-specific convenience methods
    ...
    async def validate_user_exists(self, user_id: str) -> bool:
        ...
        Used before adding users to organizations.
    ...
    async def validate_users_exist(self, user_ids: List[str]) -> dict:
        """
        ...
        Returns:
            Dict mapping user_id to exists boolean
        """
        results = {}
        for user_id in user_ids:
            results[user_id] = await self.validate_user_exists(user_id)
        return results

    async def get_user_info(self, user_id: str) -> Optional[dict]:
        """
        Get user information for organization context.
    ...
    async def get_user_email(self, user_id: str) -> Optional[str]:
        """
        Get user email for organization invitations.
    ...
    async def check_user_active(self, user_id: str) -> bool:
        ...
        Prevents organization operations for suspended/deleted accounts.
