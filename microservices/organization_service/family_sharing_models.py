...
class CreateSharingRequest(BaseModel):
    ...
    shared_with_members: Optional[List[str]] = Field(
        default_factory=list, description="共享给哪些成员（user_id列表）"
    )
    share_with_all_members: bool = Field(False, description="是否共享给所有家庭成员")
    default_permission: SharingPermissionLevel = Field(
        SharingPermissionLevel.READ_WRITE, description="默认权限级别"
    )
    custom_permissions: Optional[Dict[str, str]] = Field(
        default_factory=dict, description="自定义成员权限 {user_id: permission_level}"
    )
    quota_settings: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="配额设置"
    )
    restrictions: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="限制条件（如时间限制、使用限制等）"
    )
...
class SharedResourceDetailResponse(BaseModel):
    ...
    member_permissions: List[MemberSharingPermissionResponse] = Field(
...
class MemberSharedResourcesResponse(BaseModel):
    ...
    user_id: str
    organization_id: str
    shared_resources: List[MemberSharingPermissionResponse] = Field(
        default_factory=list
    )
    total: int
    limit: int
...
class DeviceSharingSettings(BaseModel):
    ...
    time_restrictions: Optional[
        List[Dict[str, str]]
    ] = None  # [{day: "monday", start: "18:00", end: "22:00"}]
...
class StorageSharingSettings(BaseModel):
    ...
    allocated_quotas: Dict[str, float] = Field(
...
class WalletSharingSettings(BaseModel):
    ...
    spending_limits: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="消费限额 {user_id: {daily: 100, monthly: 500}}"
    )
