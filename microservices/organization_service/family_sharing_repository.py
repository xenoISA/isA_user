...
class FamilySharingRepository:
        ...
        try:
            ...
            async with self.db:
                count = await self.db.insert_into(
                    self.sharing_table, [sharing_data], schema=self.schema
                )
    ...
    async def create_member_permission(
        ...
        try:
            async with self.db:
                count = await self.db.insert_into(
                    self.permissions_table, [permission_data], schema=self.schema
                )
            ...
            return await self.get_member_permission(
                permission_data["sharing_id"], permission_data["user_id"]
            )
        ...
        try:
            ...
            async with self.db:
                result = await self.db.query(
                    query, [sharing_id, user_id], schema=self.schema
                )
    ...
    async def get_organization_members(
        ...
        try:
            ...
            async with self.db:
                result = await self.db.query(
                    query, [organization_id, "active"], schema=self.schema
                )
    ...
    async def check_organization_member(
        ...
        try:
            ...
            async with self.db:
                result = await self.db.query(
                    query, [organization_id, user_id, "active"], schema=self.schema
                )
